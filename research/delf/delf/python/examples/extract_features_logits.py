...
TEST_DIR = '/mnt/data1/linxi/EVAL/feature/delf_global/'
CLASS_DIR = TEST_DIR + 'oxford/'
GLOBAL_DIR = CLASS_DIR + 'dataset/'
# GLOBAL_DIR = CLASS_DIR + 'query/'
# FSEL_DIR = CLASS_DIR + '/'
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      ...
      global_feat = graph.get_tensor_by_name('attonly/attention/merge/softplus_attention/Mean:0')
      ...
      for i in range(num_images):
        ...
        (locations_out, descriptors_out, feature_scales_out,
         attention_out, global_feat_out) = sess.run(
             [locations, descriptors, feature_scales, attention, global_feat],
             feed_dict={
                 input_image:
                     im,
        ...
        # print(global_feat_out.shape)
        fname = os.path.join(cmd_args.output_dir, str(i + 1) + '.mat')
        io.savemat(fname, {GLOBAL1024_FEAT: global_feat_out})
        tf.logging.info('Processing image %d , path %s', i+1, fname)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--list_images_path',
      type=str,
      default='/mnt/data1/linxi/landmarks_data/oxford5k/qoxford5k_list.txt',
      help="""
  ...
  parser.add_argument(
      '--output_dir',
      type=str,
      default='/mnt/data1/linxi/EVAL/feature/delf_global/oxford/query/',
      help="""
