...
class DelfV1(object):
  ...
  def _PerformAttention(self,
    ...
    with tf.variable_scope(
      ...
      with tf.variable_scope(
        ...
        attention_feat = tf.expand_dims(tf.expand_dims(attention_feat, 1), 2, name='att_feat')
  ...
  def GetAttentionPrelogit(
    ...
    #     feature_map = end_points[_SUPPORTED_TARGET_LAYER[0]]
    # else:
    #     feature_map = end_points[_SUPPORTED_TARGET_LAYER[1]]
...
if __name__ == '__main__':
    random_images = tf.random_normal([32, 224, 224, 3])
    net, endpoint = DelfV1().GetResnet50Subnetwork(images=random_images, is_training=True, global_pool=True)
    # attention_output = DelfV1().AttentionModel(images=random_images, num_classes=100, training_attention=True)